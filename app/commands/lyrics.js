...
module.exports = {
  name: "lyrics",
  description: "ใช้เพื่อเปิดเนื้อเพลงของเพลงนั้นๆ",
  usage: "[Song Name]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0] && !player) return client.sendTime(message.channel, "ไม่มีเพลงอะไรเล่นอยู่เด้ออ🍋");
    ...
    if (!lyrics) return client.sendTime(message.channel, `🍋หาเนื้อเพลงของเพลง:** \`${SongTitle}\`ไม่เจอ`);
    lyrics = lyrics.split("\n"); //spliting into lines
  ...
  SlashCommand: {
    options: [
      {
        ...
        value: "song",
        type: 3,
        description: "ใส่ชื่อเพลงเพื่อค้นหา",
        required: false,
      },
    ],
    /**
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!interaction.data.options && !player) return client.sendTime(interaction, "ไม่มีเพลงอะไรเล่นอยู่เด้ออ🍋");

      let SongTitle = interaction.data.options ? interaction.data.options[0].value : player.queue.current.title;
      let lyrics = await lyricsFinder(SongTitle);
      console.log(lyrics.length === 0)
      if (lyrics.length === 0)
        return client.sendTime(interaction, `🍋หาเนื้อเพลงของเพลง:** \`${SongTitle}\`ไม่เจอ`);
      lyrics = lyrics.split("\n"); //spliting into lines
      let SplitedLyrics = _.chunk(lyrics, 40); //45 lines each page
