...
module.exports = {
  name: "search",
  description: "ใช้เพื่อหาผลลัพท์ของเพลง🍋",
  usage: "[song]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    if (!message.member.voice.channel)
      return client.sendTime(
        message.channel,
        "ต้องอยู่ในช่องเสียงก่อนที่จะหา..อะไรซักอย่าง🍋"
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      let msg = await message.channel.send(
        "พิมเลขหน้าเพลงที่ต้องการเล่น🍋 จับเวลา `30 seconds`.**"
      );
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "อ่าาาา หมดเวลาก่อน พิมคําสั่งใหม่อีกครั้งนะ🍋"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(message.channel, "ส่งเลขหน้าเพลงให้ถูกต้อง🍋");
  ...
  SlashCommand: {
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "ใช้ใส่ชื่อเพลงหรือURLของเพลงเพื่อค้นหา🍋",
      },
    ],
    /**
     *
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (
        ...
        return client.sendTime(
          interaction,
          "สับสนหมดแล้วว เราต้องอยู่ช่องเสียงเดียวกันก่อนถึงจะใช้คำสั่งได้🍋"
        );
      let CheckNode = client.Manager.nodes.get(client.botconfig.Lavalink.id);
      if (!CheckNode || !CheckNode.connected) {
        return client.sendTime(
