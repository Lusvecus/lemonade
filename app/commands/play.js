...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        if (!message.member.voice.channel) return client.sendTime(message.channel, "❌ต้องเข้าช่องที่จะฟังเพลงก่อนที่จะฟัง อะไร..บางอย่าง🍋");
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    return client.sendTime(message.channel, "ไม่เจอ" + SearchString);
                }
            } else {
                let Searched = await player.search(SearchString, message.author);
                if (!player) return client.sendTime(message.channel, "ไม่มีเพลงอะไรเล่นอยู่เด้ออ🍋");

                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "ไม่เจอ" + SearchString);
        ...
        options: [
            {
                ...
                type: 3,
                required: true,
                description: "ใช้เพื่อเล่นเพลงในช่องเสียง🍋",
            },
        ],
        /**
         *
        ...
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                switch (Searched.loadType) {
                    ...
                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                        return client.sendTime(interaction, "ไม่เจอเพลงอ่ะ;-;");
                    case "TRACK_LOADED":
